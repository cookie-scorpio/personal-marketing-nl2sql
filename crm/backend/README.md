...
本目录是个金营销 NL2SQL 平台的 Spring Boot v1.6 后端。登录、角色授权、身份切换、自然语言解析、客户定位、SQL规划、查询执行、会话和审计位于同一进程，各模块按业务包隔离，通过 Java 接口直接调用。

v1.6增加五位工号注册、待审批账号、多身份授权、身份切换、质量审计问数和权限管理。客户经理查询仍按经理、机构或区域限定；质量审计员查询强制带有全部在册客户谓词；权限管理员不能访问业务问数。每个SQL候选都会重新经过账号权限、客户范围、只读AST和风险确认。完整字段与接口见[实施说明](../../docs/v1.6实施说明与接口数据字典.md)，数据库迁移按顺序保留为 V1–V18。
...
- 五位工号、必填姓名和唯一用户名注册；用户名、工号由服务端预检查并由数据库唯一索引兜底，账号审批后才能登录。
- 客户经理、质量审计员和权限管理员多身份授权，切换时重新读取授权并签发JWT。
- 权限清单直接展示注册姓名并按工号升序排列；权限管理员可删除其他账号及其角色授权，但不能删除自己，历史会话和审计记录继续保留。
- 每次 JWT 认证都会复核账号仍存在且处于启用状态，账号删除后旧令牌立即失效。
- 客户经理按经理、机构或区域限定；质量审计员仅查询全部在册客户，权限管理员不具备问数范围。
- 会话按用户和当前身份共同隔离，同一账号切换身份后不会读取另一身份的标题、消息或任务。
...
- `POST /api/v1/auth/switch-identity`：复核目标身份并签发新的当前身份JWT。
- `GET /api/v1/permission-admin/accounts`：按工号读取账号姓名、状态和授权清单。
- `PUT /api/v1/permission-admin/accounts/{accountId}/permissions`：覆盖目标账号的身份和业务范围。
- `DELETE /api/v1/permission-admin/accounts/{accountId}`：删除其他用户的账号与角色授权，禁止删除当前登录账号。
...
详细字段、接口示例、当前限制和升级方式见 [v1.6实施说明](../../docs/v1.6实施说明与接口数据字典.md)。
