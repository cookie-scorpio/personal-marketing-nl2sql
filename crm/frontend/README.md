...
v1.2需配套同版本后端。默认代理到8080；后端更换端口时，在`.env.development.local`中设置`API_PROXY_TARGET=http://127.0.0.1:18080`并重启Vite。本机因Windows保留8080所在端口段，已配置为18080。配置仅影响开发代理，不改变生产环境。完整协议见[实施说明](../../docs/v1.2实施说明与接口数据字典.md)。
...
数据字典、完整API示例及未实现项见 [v1.2实施说明](../../docs/v1.2实施说明与接口数据字典.md)。
