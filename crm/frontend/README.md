...
- 五位工号、必填姓名和唯一用户名注册；重复用户名明确提示修改，申请提交后进入待审批状态。
- 质量审计员可在顶栏切换“智能问数 / 后台管理”；客户经理与审计员会话按当前身份分别加载。
- 身份切换销毁旧身份的SSE、草稿和组件状态，在当前页面重建工作区，不触发浏览器整页跳转。
- 左下账号信息统一展示当前用户的五位工号；工号属于账号，不随客户经理、质量审计员或权限管理员身份变化。
- 权限管理员虽不展示会话列表，账号信息、身份卡和退出按钮仍与其他身份保持相同的侧栏左下位置。
- 权限管理清单直接显示注册姓名，按五位工号升序排列并使用主色表头；操作列居中展示“配置权限”和“删除用户”。
- 权限管理员编辑本人授权时不能取消自身的权限管理员身份；选项以禁用态展示，后端同时拒绝绕过界面的撤权请求。
- 删除用户前显示包含姓名和用户名的二次确认；当前登录账号的删除入口灰显且后端拒绝自删。
...
- 侧栏账号信息展示当前用户的五位工号，身份名称在下方身份卡单独显示。页面不展示静态伪造的经营指标，查询结果来自后端和MySQL。
