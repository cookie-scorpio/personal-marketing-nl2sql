...
public class CustomerQueryPlanner {
    ...
    public Optional<PlannedQuery> plan(String text,String customer,CurrentUser user){
        ...
        if(Set.of("产品持有","产品持有情况","持有产品","持仓").contains(simple))return Optional.of(new PlannedQuery(
                "SELECT c.customer_id,c.customer_name_masked AS customer_name,h.product_name,h.product_category_code,ROUND(h.market_value_amount/10000,2) AS market_value_wan,h.snapshot_date FROM dim_customer c JOIN fct_product_holding h ON h.customer_id=c.customer_id WHERE "+where+" ORDER BY h.market_value_amount DESC,h.holding_id",args,"AUTO","客户产品持有",false));
