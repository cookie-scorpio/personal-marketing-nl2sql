...
param(
    ...
    [string]$Database = 'pf_nl2sql_v15_test',
    [string]$BudgetFile = '',
    [string]$CaseIds = ''
)
...
try {
    ...
    if ($ApiRules -or $RealModel) {
        ...
        & $Python @evalArgs
        if ($LASTEXITCODE -ne 0) { throw 'HTTP evaluation failed; see tmp/v15/evaluation.' }
    }
    Write-Host 'Selected v1.5 checks passed. Unselected MySQL, API, or real-model checks were not run.'
} finally { Pop-Location }
