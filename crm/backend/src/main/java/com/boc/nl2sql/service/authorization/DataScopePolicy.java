...
public class DataScopePolicy {
    ...
    public static Scope scopeOf(CurrentUser user) {
         ...
         */
        if (user.role().normalized() == com.boc.nl2sql.domain.authorization.RoleCode.QUALITY_AUDITOR) {
            return new Scope("status_code", "ACTIVE", "scopeCustomerStatus");
        }
        com.boc.nl2sql.domain.authorization.BusinessDataScopeLevel level = user.businessScopeLevel();
