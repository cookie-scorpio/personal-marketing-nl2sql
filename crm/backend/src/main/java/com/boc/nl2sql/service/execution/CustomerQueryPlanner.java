...
public class CustomerQueryPlanner {
    ...
    public CustomerQueryPlanner(DataScopePolicy scope) {
        this.scope = scope;
    }
    ...
    public Optional<PlannedQuery> plan(String text, String customer, CurrentUser user) {
        ...
        String where = scope.condition("c", user, args)
                + " AND c.status_code='ACTIVE' AND c.customer_id=:resolvedCustomerId";
