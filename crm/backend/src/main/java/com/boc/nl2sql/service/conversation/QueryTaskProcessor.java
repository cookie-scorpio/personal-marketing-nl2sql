...
import com.boc.nl2sql.service.execution.*;
import com.boc.nl2sql.domain.execution.*;
import com.boc.nl2sql.model.ModelGateway;
import com.boc.nl2sql.model.QueryInterpretation;
import com.boc.nl2sql.service.nl2sql.CompletenessValidator;
...
import com.boc.nl2sql.dao.quality.RepairFactStore;
...
public class QueryTaskProcessor {
    ...
    public QueryTaskProcessor(QueryTaskMapper taskMapper, TaskStateStore states, ModelGateway modelGateway,
            ...
            com.boc.nl2sql.service.nl2sql.DisplayConflictGuard conflictGuard,
    ...
    private final com.boc.nl2sql.service.nl2sql.DisplayConflictGuard conflictGuard;
    ...
    public void processAsync(String taskId, CurrentUser user, String requestId) {
        ...
        try (var context=com.boc.nl2sql.model.ModelCallContext.open(()->states.active(taskId),task::getResolvedCustomerId)) {
            ...
            if(customerList!=null){
                ...
                var ctx=task.getContextJson()==null?null:json.readValue(task.getContextJson(),com.boc.nl2sql.domain.conversation.ConversationContext.class);
                if(followups.followup(task.getMergedQueryText()) && !followups.selfContained(task.getMergedQueryText()) && task.getResolvedCustomerId()==null && (ctx==null||ctx.query().isBlank())){
                    ask(task,new com.boc.nl2sql.domain.nl2sql.ClarificationQuestion(UUID.randomUUID().toString(),"FOLLOWUP_CONTEXT","本会话还没有可以承接的查询，请完整描述本次查询对象和指标。",List.of(),Map.of()),requestId);return;
                }
                String compareTag="";
