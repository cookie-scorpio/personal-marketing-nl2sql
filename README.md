...
当前为 v1.0：Vue 工作台与 Spring Boot 后端已完成登录、规则与 DeepSeek 混合识别、信息补充、矛盾澄清、SQL 风险确认、MySQL 查询、可视化分析和历史记录。高频问题优先走规则，自由问题在配置 DeepSeek API 后使用大模型。Milvus 与 BCG-E3 暂只保留外置接口。

完整的实现范围、未实现项、数据库字段中文释义和 API 示例见 [v1.0 实施说明与接口数据字典](docs/v1.0实施说明与接口数据字典.md)。
