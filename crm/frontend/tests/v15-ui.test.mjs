...
test('业务主页面使用混合流式展示且不影响历史会话', async () => {
  ...
  assert.match(result, /<template v-else>[\s\S]*<el-table[\s\S]*class="result-structured-enter"/)
...
test('查询结果默认二十条并通过原任务安全翻页', async () => {
  const workspace = await readFile(new URL('../src/features/conversation/ConversationWorkspace.vue', import.meta.url), 'utf8')
  const result = await readFile(new URL('../src/features/conversation/QueryResultView.vue', import.meta.url), 'utf8')
  const messageText = await readFile(new URL('../src/features/conversation/messageText.ts', import.meta.url), 'utf8')
  const styles = await readFile(new URL('../src/app/styles.css', import.meta.url), 'utf8')
  // 首次查询主动传入 20 条；翻页复用 task_id，不重复提交自然语言查询或新增聊天消息。
  assert.match(workspace, /const QUERY_RESULT_DEFAULT_PAGE_SIZE = 20/)
  assert.match(workspace, /page_size: QUERY_RESULT_DEFAULT_PAGE_SIZE/)
  assert.match(workspace, /:task-id="message\.task_id"/)
  assert.match(result, /\/queries\/\$\{props\.taskId\}\/results\?page_no=\$\{pageNo\}&page_size=\$\{pageSize\}/)
  assert.match(result, /const pageSizes = \[10, 20, 50, 100\]/)
  assert.match(messageText, /export const QUERY_RESULT_MAX_PAGES = 50/)
  assert.match(result, /currentPageSize\.value \* QUERY_RESULT_MAX_PAGES/)
  assert.match(result, /:total="paginationTotal"/)
  assert.match(result, /最多查看前 \{\{ format\(paginationTotal\) \}\} 条（\{\{ QUERY_RESULT_MAX_PAGES \}\} 页）/)
  assert.match(result, /<el-pagination/)
  assert.match(result, /@current-change="changePage"/)
  assert.match(result, /@size-change="changePageSize"/)
  // 在途请求可取消且只有最后一次响应生效；失败时保留当前页并提供原位重试。
  assert.match(result, /pageRequest\?\.abort\(\)/)
  assert.match(result, /requestVersion !== pageRequestVersion/)
  assert.match(result, /class="result-page-error" role="alert"/)
  assert.match(result, />重新加载<\/el-button>/)
