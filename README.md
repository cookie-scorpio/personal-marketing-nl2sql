...
当前为 v1.2：支持连续对话、SSE阶段反馈、运行中取消、默认开启的思考开关、客户身份澄清、多轮上下文和持久化会话；增加提交幂等、并发保护、复杂MySQL SQL的AST校验及独立SQL核查日志。高频明确问题优先走规则，自由问题在配置DeepSeek后使用大模型。Milvus与BCG-E3仍只保留外置接口。

实现范围、未实现项、全部数据库字段、API示例、日志核查和验收记录见 [v1.2实施说明与接口数据字典](docs/v1.2实施说明与接口数据字典.md)。[v1.0](docs/v1.0实施说明与接口数据字典.md)、[v1.1](docs/v1.1实施说明与接口数据字典.md)保留作历史参考。
...
SQL默认执行超时为60秒，通过`QUERY_EXECUTION_TIMEOUT_SECONDS`配置。升级时需要部署同版本前后端，Flyway自动执行V5；先停止旧版本任务，避免旧进程覆盖新版取消状态。取消或超时后不会继续修复或执行模板。
...
先启动 MySQL 和 Redis，再部署后端 Maven 生成的可执行 JAR，最后将前端 `npm run build` 生成的 `dist/` 交给静态服务器。静态服务器需将 `/api` 和 `/actuator/health` 反向代理到后端。密钥和环境差异通过外部配置注入。SSE代理需关闭缓冲，读取超时大于55秒。SQL日志默认写到后端工作目录的`logs/sql-review.log`，部署时提供持久化目录。具体命令见前后端说明。
